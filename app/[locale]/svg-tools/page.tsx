...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const path = '/svg-tools'
  const canonical = getCanonicalUrl(path, locale)
  const alternates = languageAlternates(path)
  const ogLocale = getOgLocale(locale)
  const baseUrl = getBaseUrl()

  const title = 'SVG conversion tools'
  const description =
    'Browse all free SVG image conversion tools. Convert SVG images to PNG, JPG, WebP and more directly in your browser.'
...
export default async function SvgToolsPage({ params }: Props) {
  const { locale } = await params
  ...
  return (
    <FormatHubPage
      ...
      sourceFormat="svg"
      title="SVG tools"
      description="All SVG-related image conversion tools in one place."
    />
  )
}
