...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const path = '/heic-tools'
  const canonical = getCanonicalUrl(path, locale)
  const alternates = languageAlternates(path)
  const ogLocale = getOgLocale(locale)
  const baseUrl = getBaseUrl()

  const title = 'HEIC conversion tools'
  const description =
    'Browse all free HEIC image conversion tools. Convert HEIC images to JPG, PNG, WebP and more directly in your browser.'
...
export default async function HeicToolsPage({ params }: Props) {
  const { locale } = await params
  ...
  return (
    <FormatHubPage
      ...
      sourceFormat="heic"
      title="HEIC tools"
      description="All HEIC-related image conversion tools in one place."
    />
  )
}
