...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const path = '/gif-tools'
  const canonical = getCanonicalUrl(path, locale)
  const alternates = languageAlternates(path)
  const ogLocale = getOgLocale(locale)
  const baseUrl = getBaseUrl()

  const title = 'GIF conversion tools'
  const description =
    'Browse all free GIF image conversion tools. Convert GIF images to JPG, PNG, WebP and more directly in your browser.'
...
export default async function GifToolsPage({ params }: Props) {
  const { locale } = await params
  ...
  return (
    <FormatHubPage
      ...
      sourceFormat="gif"
      title="GIF tools"
      description="All GIF-related image conversion tools in one place."
    />
  )
}
