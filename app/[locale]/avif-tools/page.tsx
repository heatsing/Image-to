...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const path = '/avif-tools'
  const canonical = getCanonicalUrl(path, locale)
  const alternates = languageAlternates(path)
  const ogLocale = getOgLocale(locale)
  const baseUrl = getBaseUrl()

  const title = 'AVIF conversion tools'
  const description =
    'Browse all free AVIF image conversion tools. Convert AVIF images to JPG, PNG, WebP and more directly in your browser.'
...
export default async function AvifToolsPage({ params }: Props) {
  const { locale } = await params
  ...
  return (
    <FormatHubPage
      ...
      sourceFormat="avif"
      title="AVIF tools"
      description="All AVIF-related image conversion tools in one place."
    />
  )
}
