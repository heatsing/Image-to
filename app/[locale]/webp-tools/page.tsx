...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const path = '/webp-tools'
  const canonical = getCanonicalUrl(path, locale)
  const alternates = languageAlternates(path)
  const ogLocale = getOgLocale(locale)
  const baseUrl = getBaseUrl()

  const title = 'WebP conversion tools'
  const description =
    'Browse all free WebP image conversion tools. Convert WebP images to JPG, PNG, AVIF, GIF, SVG and more directly in your browser.'
...
export default async function WebpToolsPage({ params }: Props) {
  const { locale } = await params
  ...
  return (
    <FormatHubPage
      ...
      sourceFormat="webp"
      title="WebP tools"
      description="All WebP-related image conversion tools in one place."
    />
  )
}
