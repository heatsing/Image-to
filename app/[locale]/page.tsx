...
import { getPopularConversionSlugs } from '@/lib/seo/page-content'
...
export default async function HomePage({ params }: Props) {
  ...
  const messages = getMessages(locale)
  const baseUrl = getBaseUrl()
  const popularConversions = getPopularConversionSlugs(9)
    ...
    <div className="bg-slate-50 min-h-screen flex flex-col">
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl flex-1">
        ...
        <RelatedLinksSection
          ...
          description="These are the highest-intent routes on the site and the best places to start if you already know the source format."
          locale={locale}
          slugs={popularConversions}
        />
        <GuideLinksSection
          title="Format guides and strategy"
          description="Use these editorial guides when you need to choose the right format or fix a workflow problem before converting."
          locale={locale}
          slugs={featuredGuides}
        />
        <TopicLinksSection
          title="Workflow topics and fixes"
          description="These pages target practical tasks like shrinking files, fixing HEIC uploads, handling screenshots, and preparing assets for publishing."
          locale={locale}
