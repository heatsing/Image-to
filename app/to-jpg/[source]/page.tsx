...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${from} to JPG Converter - Free Online Tool`
  const description = `100% free. Convert ${from} to JPG locally—no uploads, no signup. Your files never leave your device.`
  return { title, description }
}

export default async function ToJPGSourcePage({ params }: Props) {
  const { source } = await params
  if (!isValidFormatSlug(source)) notFound()

  const from = slugToLabel(source)
  const title = `${from} to JPG Converter`
  const desc = `100% free. Convert ${from} to JPG locally—no uploads, no signup. Your files never leave your device.`
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl flex-1">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-600 mb-3">
            100% free. Convert {from} to JPG <strong>locally</strong>—no uploads, no signup. Your files never leave your device.
          </p>
        </div>
